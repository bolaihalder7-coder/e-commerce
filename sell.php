...
$stmt = $conn->prepare("SELECT * FROM new_products WHERE id = ?");
...
if(!$product){
    echo "<h3 style='color:orange; text-align:center;'>Product not found! Please select another product.</h3>";
    exit;
}
?>
<!DOCTYPE html>
...
<div class="container mt-5">
    ...
    <form action="process_buy.php?id=<?= $product['id'] ?>" method="POST">
        ...
        <div class="mb-3">
            ...
            <input type="text" class="form-control" name="product_name" value="<?= htmlspecialchars($product['name']) ?>" >
        </div>
